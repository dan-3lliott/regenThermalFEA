...
%get ready to export each frame as a video
images = cell(timeSteps + 1,1);
slowMoFactor = 10; %make the video take [slowMoFactor]x as long as realtime
...
%start the image array at t=0
plotMesh(nodes, elements, D);
images{1} = frame2im(getframe);

%compute temperature matrix with forward time steps
for i = 1:timeSteps
    Dprev = D;
    Rprev = R;
    ...
    D = lhs\rhs;
    plotMesh(nodes, elements, D);
    images{i+1} = frame2im(getframe);
    disp(i/timeSteps);
    avgTemp = [avgTemp mean(D)];
end

%export result of finest timestep
exportVideo(images(2:end), dTime, "Transient.avi", slowMoFactor);
...
%function for compiling and exporting the video
function exportVideo(images, dTime, fileName, slowMoFactor)
    writerObj = VideoWriter(fileName);
    writerObj.FrameRate = (1/dTime);
    open(writerObj);
    for i = 1:length(images)
        for j = 1:slowMoFactor
            writeVideo(writerObj, im2frame(images{i}));
        end
    end
    close(writerObj);
end